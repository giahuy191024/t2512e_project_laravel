...
<style>
    ...
    /* Overlay gradient phía dưới ảnh */
    .dc-photo-overlay {
        position: absolute;
        inset: 0;
        background: linear-gradient(to bottom, transparent 40%, rgba(13,71,161,.75) 100%);
    }

    /* Badge chuyên khoa nổi trên ảnh */
    .dc-spec-badge {
        position: absolute;
        top: 12px;
        left: 12px;
    ...
    /* Tên bác sĩ nổi trên overlay ảnh */
    .dc-name-overlay {
        position: absolute;
        bottom: 12px;
        left: 14px;
    ...
    .btn-book:hover::after { background: rgba(255,255,255,.08); }
    ...
    /* Stagger delay cho từng card */
    .doctor-col:nth-child(1)  .doctor-card { animation-delay: .05s; }
    .doctor-col:nth-child(2)  .doctor-card { animation-delay: .10s; }
    .doctor-col:nth-child(3)  .doctor-card { animation-delay: .15s; }
    .doctor-col:nth-child(4)  .doctor-card { animation-delay: .20s; }
    .doctor-col:nth-child(5)  .doctor-card { animation-delay: .25s; }
    .doctor-col:nth-child(6)  .doctor-card { animation-delay: .30s; }
    .doctor-col:nth-child(7)  .doctor-card { animation-delay: .35s; }
    .doctor-col:nth-child(8)  .doctor-card { animation-delay: .40s; }
    .doctor-col:nth-child(9)  .doctor-card { animation-delay: .45s; }
</style>
@endpush
...
<div class="doctors-hero">
    ...
    <p>Chọn chuyên khoa và thành phố để tìm bác sĩ phù hợp với bạn</p>
</div>
    ...
    <form action="{{ route('patient.doctors') }}" method="GET">
        <div class="row align-items-end">
            ...
            <div class="col-md-3 form-group mb-md-0">
                <label for="specialization_id"><i class="fas fa-stethoscope mr-1"></i> Chuyên khoa</label>
...
<div class="result-count mb-3">
    ...
    @if(request()->hasAny(['keyword','specialization_id','city_id']))
        phù hợp với bộ lọc
    @endif
</div>
        ...
        <div class="col-xl-3 col-lg-4 col-md-6 mb-4 doctor-col">
            <div class="doctor-card">
                ...
                <div class="dc-photo-wrap">
                    {{-- Dùng pravatar.cc để có ảnh người thật, seed = id bác sĩ cho nhất quán --}}
                    @php
                        $photoId = ($d->id % 70) + 1;
                        $gender  = ($d->id % 2 === 0) ? 'men' : 'women';
                    @endphp
                    <img
                        src="https://randomuser.me/api/portraits/{{ $gender }}/{{ $photoId }}.jpg"
                        alt="Bác sĩ {{ $d->full_name }}"
                        loading="lazy"
                        onerror="this.src='https://ui-avatars.com/api/?name={{ urlencode($d->full_name) }}&background=1a73e8&color=fff&size=300&bold=true&font-size=0.4'"
                    >
                    <div class="dc-photo-overlay"></div>

                    {{-- Badge chuyên khoa --}}
                    <div class="dc-spec-badge">
                        <i class="fas fa-tooth"></i>{{ $d->specialization->name ?? 'Chuyên khoa' }}
                    </div>

                    {{-- Tên nổi trên ảnh --}}
                    <div class="dc-name-overlay">
                        <h4>BS. {{ $d->full_name }}</h4>
                        <span class="dc-title">{{ $d->qualifications ?? 'Bác sĩ chuyên khoa' }}</span>
                    </div>
                </div>
                ...
                <div class="dc-body">
                    ...
                    <div class="dc-info-row">
                        <div class="dc-chip">
                            ...
                            <span style="overflow:hidden;white-space:nowrap;text-overflow:ellipsis">{{ $d->city->name ?? '—' }}</span>
                        </div>
                        <div class="dc-chip">
                            <i class="fas fa-phone-alt text-primary"></i>
                            <span>{{ $d->phone_number ? 'Có lịch' : 'Liên hệ' }}</span>
                        </div>
                    </div>
