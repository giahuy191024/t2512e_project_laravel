...
@section('content')
    ...
    {{-- Stats Row --}}
    <div class="stats-row">
        <div class="stat-chip chip-all {{ !request('status') && request('status') !== '0' ? 'active' : '' }}" onclick="filterByStatus('')">
            <div>
                <div class="label">Tất cả</div>
                <div class="value">{{ $stats['total'] }}</div>
            </div>
            <i class="fas fa-tooth" style="font-size:24px;color:#6c757d;opacity:.3"></i>
        </div>
    ...
    <div class="filter-card">
        <form method="GET" action="{{ route('admin.specialties') }}">
            <div class="filter-row">
                <div class="filter-group">
                    <label><i class="fas fa-search"></i> Tìm kiếm</label>
                    <input type="text" name="search" class="filter-input" placeholder="Tên chuyên khoa..." value="{{ request('search') }}">
                </div>
                ...
                <div style="display:flex;gap:8px">
                    ...
                    <a href="{{ route('admin.specialties') }}" class="btn btn-secondary btn-filter" style="background:#adb5bd"><i class="fas fa-redo"></i> Reset</a>
                </div>
            </div>
        </form>
    </div>
    ...
    <div class="card">
        <div class="card-header">
            <h3 class="card-title mt-1">
                <i class="fas fa-tooth"></i> Danh sách Chuyên khoa
                <span class="badge badge-secondary ml-2">{{ $specialties->count() }} kết quả</span>
            </h3>
            ...
            <table class="table table-hover table-striped text-nowrap">
                ...
                @forelse($specialties as $sp)
                    <tr>
                        ...
                        <td class="align-middle text-center"><span class="icon-emoji">{{ $sp->icon ?? '🦷' }}</span></td>
                        <td class="align-middle"><span class="font-weight-bold">{{ $sp->name }}</span></td>
                        <td class="align-middle"><code>{{ $sp->slug }}</code></td>
                        <td class="align-middle text-muted">{{ Str::limit($sp->description, 50) ?? '—' }}</td>
                        <td class="align-middle text-center">
                            <span class="badge badge-info">{{ $sp->doctors_count }}</span>
                        </td>
                        <td class="align-middle text-center">{{ $sp->sort_order }}</td>
                        <td class="align-middle text-center">
                            @if($sp->status == 1)
                        ...
                        <td class="text-right align-middle">
                            <a href="{{ route('admin.specialties.edit', $sp->id) }}" class="btn btn-sm btn-info" title="Chỉnh sửa">
                                <i class="fas fa-pen"></i>
                            </a>
                            <form action="{{ route('admin.specialties.delete', $sp->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Xóa chuyên khoa này?');">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-sm btn-danger" title="Xóa" {{ $sp->doctors_count > 0 ? 'disabled' : '' }}>
